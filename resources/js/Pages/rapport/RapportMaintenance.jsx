...
import { FaArrowLeft, FaUser, FaPrint, FaCheck, FaInfo, FaTools, FaListAlt, FaCalendarAlt, FaBars, FaTimes } from "react-icons/fa";
...
const RapportMaintenance = ({ intervention_id }) => {
            ...
            <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-900">
                <div className="max-w-md p-8 bg-white shadow-xl dark:bg-slate-800 rounded-xl">
                    <div className="flex flex-col items-center">
                        ...
                        <h2 className="mb-3 text-xl font-semibold text-slate-800 dark:text-slate-200">Erreur</h2>
                        <p className="mb-6 text-center text-slate-600 dark:text-slate-400">{error}</p>
    ...
    const TabButton = ({ id, label, icon }) => (
        <button
            onClick={() => {
                setActiveTab(id);
                setIsSidebarOpen(false); // Fermer le drawer sur mobile après sélection
            }}
            className={`w-full flex items-center px-4 py-3 text-left rounded-lg transition-colors ${activeTab === id
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200">
            ...
            {isLoading ? (
                ...
                <>
                    {/* Fixed Header */}
                    <div className={`fixed top-0 left-0 right-0 z-30 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 ${isPrinting ? 'print-header' : ''}`}>
                        <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
                            <div className={`py-4 flex flex-col md:flex-row md:items-center md:justify-between ${isPrinting ? 'print-header' : ''}`}>
                                ...
                                <div className="flex flex-col items-end">
                                    ...
                                    <div className="mt-1 text-lg font-semibold">Rapport d'intervention #{rapport?.id}</div>
                                    <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">{formatDate(rapport?.date_intervention)}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-1 pt-24 md:pt-32">
                        ...
                        <div className={`flex-1 md:ml-64 ${isPrinting ? 'print-content' : ''}`}>
                            <div className="h-full overflow-y-auto">
                                <div className="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
                                    {/* Content */}
                                    <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-xl overflow-hidden ${isPrinting ? 'print-content' : ''}`}>
                                        {/* Information Client */}
                                        ...
                                        {activeTab === 'probleme' && (
                                            <div className={`p-6 ${isPrinting ? 'print-section' : ''}`}>
                                                ...
                                                <div className="space-y-6">
                                                    <div>
                                                        <h3 className="mb-3 text-lg font-medium">Problèmes rapportés</h3>
                                                    ...
                                                    {rapport?.photo_probleme?.length > 0 && (
                                                        <div>
                                                            <h3 className="mb-3 text-lg font-medium">Documentation visuelle</h3>
                                                            <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 justify-center bg-slate-50 dark:bg-slate-700/40 rounded-lg p-4 ${isPrinting ? 'print-card' : ''}`}>
                                                                {JSON.parse(rapport?.photo_probleme).map((path, index) => (
                                                                    <img
                                                                        key={index}
                                                                        src={`/${path}`}
                                        ...
                                        {activeTab === 'diagnostic' && (
                                            <div className={`p-6 ${isPrinting ? 'print-section' : ''}`}>
                                                ...
                                                <div className="space-y-6">
                                                    <div>
                                                        <h3 className="mb-3 text-lg font-medium">Diagnostic initial</h3>
                                                    ...
                                                    <div>
                                                        <h3 className="mb-3 text-lg font-medium">Cause identifiée</h3>
