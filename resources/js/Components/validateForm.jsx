export const validateFormClient = (data, setValidationErrors) => {
    ...
    if (!data.surface_cultivee) {
        ...
        errors.surface_cultivee =
            "La surface cultivée doit être un nombre positif.";
    }
    if (!data.type_activite_agricole.trim()) {
        errors.type_activite_agricole =
            "Le type d'activité agricole est obligatoire.";
    }
...
export const validateFormInstalation = (data, setValidationErrors) => {
    ...
    if (!data.puissance_pompe.trim()) {
        ...
        errors.puissance_pompe =
            "La puissance de pompe doit être un nombre positif.";
    }
    if (!data.profondeur_forage.trim()) {
        errors.profondeur_forage =
            "La profondeur de la forage est obligatoire.";
    } else if (isNaN(data.profondeur_forage) || data.profondeur_forage <= 0) {
        errors.profondeur_forage =
            "La profondeur de la forage doit être un nombre positif.";
    }
...
export const validateFormPaiement = (data, setValidationErrors) => {
    ...
    if (data.client_id <= 0) {
        errors.prenom_acheteur = "Le prenom client est obligatoire.";
    }
    if (!data.montant_paye.trim()) {
        errors.montant_paye = "Le montant est obligatoire.";
    } else if (isNaN(data.montant_paye) || data.montant_paye <= 0) {
        errors.montant_paye = "Le montant doit être un nombre positif.";
    }
    ...
    if (!data.objet.trim()) {
        errors.objet = "L'observation sont obligatoires.";
    }

    // Validation des produits
    if (!data.produits || data.produits.length === 0) {
        errors.produits = "Ajouter au moins un produit ou service.";
    } else {
        ...
        data.produits.forEach((produit, index) => {
            ...
            if (!produit.designation || !produit.designation.trim()) {
                produitErrors.designation = "La désignation est obligatoire.";
            }

            if (!produit.prix_unitaire && produit.prix_unitaire !== 0) {
                produitErrors.prix_unitaire =
                    "Le prix unitaire est obligatoire.";
            } else if (
                isNaN(produit.prix_unitaire) ||
            ...
            if (Object.keys(produitErrors).length > 0) {
                produitsErrors[index] = produitErrors;
            }
        });

        if (produitsErrors.length > 0) {
            errors.produits = produitsErrors;
        }
    }
...
export const validateFormMaintenance = (data, setValidationErrors) => {
    ...
    if (data.installation_id <= 0) {
        errors.installation_id =
            "Le nom client avec date d'installation est obligatoire.";
    }
