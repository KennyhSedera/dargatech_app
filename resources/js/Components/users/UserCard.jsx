...
const UserCard = ({ user, handleDelete, handleImageError }) => {
    return (
        <div key={user.id} className="overflow-hidden transition-all duration-300 transform bg-white shadow-lg dark:bg-gray-800 rounded-xl hover:scale-105 hover:shadow-xl">
            <div className="relative">
                ...
                <div className="relative flex items-start justify-between px-6 pt-4 pb-14">
                    ...
                    <div className="relative">
                        {!user.user_role.isAdmin && <button onClick={() => handleDelete(user.id)} className="p-2 text-white transition-colors rounded-full hover:bg-red-50 dark:hover:bg-gray-700">
                            <FaTrash />
                        </button>}
                    </div>
                </div>
